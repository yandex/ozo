...
struct shared_wrapper {
    ...
    friend void asio_handler_invoke(Function&& f, shared_wrapper* ctx) {
        using boost::asio::asio_handler_invoke;
        asio_handler_invoke(std::forward<Function>(f), ctx->ptr.get());
    }
};
...
struct io_context : asio::execution_context {
    ...
    io_context(const executor_mock& executor, const strand_executor_service_mock& strand_service)
        : executor {&executor}, strand_service_(&strand_service) {}
...
//    template <typename Function>
//    friend void asio_handler_invoke(Function&& f, callback_handler* ctx) {
