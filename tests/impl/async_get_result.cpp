...
TEST_F(async_get_result_op, perform_should_post_continuation_within_handler_executor) {
    const InSequence s;
...
TEST_P(async_get_result_op_call_with_error, should_post_callback_with_given_error) {
    ...
    EXPECT_CALL(m.callback_executor, post(_)).WillOnce(InvokeArgument<0>());
...
TEST_P(async_get_result_op_call_with_error, should_set_query_state_in_error) {
    ...
    EXPECT_CALL(m.socket, cancel(_)).WillOnce(Return());
    EXPECT_CALL(m.callback_executor, post(_)).WillOnce(Return());
