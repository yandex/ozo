...
template <typename C, typename TimeConstrain, typename Handler>
inline void async_connect(std::string conninfo, TimeConstrain&& time_constrain,
    ...
    ).perform(conninfo, std::forward<TimeConstrain>(time_constrain));
}
